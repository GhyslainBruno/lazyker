...
import {ShowTorrent, TaggedTorrents} from '../../Shows';
...
type ShowsTorrentsListProps = {
  torrents: any;
  downloadTorrent: (torrent: ShowTorrent) => void;
}

const ShowsTorrentsList = (props: ShowsTorrentsListProps) => {

  const [uhd, setUhd] = useState(false);
  const [hd, setHd] = useState(false);
  const [fullHd, setFullHd] = useState(false);
  const [multi, setMulti] = useState(false);
  const [torrents, setTorrents] = useState<TaggedTorrents>(props.torrents);
  const [torrentsFiltered, setTorrentsFiltered] = useState(props.torrents);
      ...
      const localFilteredTorrents = torrentsProvider.torrents.map(torrent => {
        ...
        if (trueFilter.length > 0) {
          trueFilter.map(filter => {
            ...
            const tagsNames = Object.keys(torrent.tags);

            // Filter the tags names by the only ones that are set to true
            // @ts-ignore
            const tagsNamesSetToTrue = tagsNames.filter((key) => torrent.tags[key]);

            // Check if the torrent tags contains the tag selected in UI
            const doTorrentTagsContainTrueFilterTag = tagsNamesSetToTrue.includes(filter);

            if (doTorrentTagsContainTrueFilterTag) {
...
export default ShowsTorrentsList;
